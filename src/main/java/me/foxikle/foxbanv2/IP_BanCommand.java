...
public class IP_BanCommand implements CommandExecutor, TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (label.equalsIgnoreCase("fip-ban")) {
            if (sender instanceof Player) {
                ...
                if (getRank(banner).getPowerLevel() >= 80) {
                    if (args.length < 4) {
                        ...
                        banner.sendMessage(ChatColor.RED + "Incorrect usage: /ip-ban <PLAYER> <REASON> <DURATION> <SILENT/PUBLIC>");
                    }
                } else {
    ...
    private void banPlayer(Player banner, Player banee, boolean silent, String reasonStr, String duration, String broadcastReason, String serverName, String appealLink){
        ...
        if(!duration.equalsIgnoreCase("PERMANANT")) {
            ...
            Bukkit.getBanList(BanList.Type.IP).addBan(banee.getName(), bumper + ChatColor.RED + "You are permanantly banned from " + serverName + ". \n" + ChatColor.AQUA + "Reason: " + ChatColor.RESET + reasonStr + ChatColor.AQUA + "\n Your ban will last indefinetly." + bumper, expires, banner.getName());
        ...
        if(!broadcastReason.equalsIgnoreCase("SECURITY")) {
            if (silent) {
                ...
                banner.sendMessage(ChatColor.RED + "" +  ChatColor.BOLD +  banee.getName() + " was IP-banned silently for " + reasonStr + ".");
            } else {
                Bukkit.broadcastMessage(ChatColor.RED + "" + ChatColor.BOLD + banee.getName() + " was removed from your game for " + broadcastReason);
                banner.getWorld().playSound(banner.getLocation(), Sound.ENTITY_LIGHTNING_BOLT_THUNDER, 1000f, 1);
                banner.sendMessage(ChatColor.RED + "" +  ChatColor.BOLD +  banee.getName() + " was IP-banned for " + reasonStr + ".");
            }
        } else{
            banner.sendMessage(ChatColor.RED + "" +  ChatColor.BOLD +  banee.getName() + " was IP-banned for SECURITY.");
        }
    }
